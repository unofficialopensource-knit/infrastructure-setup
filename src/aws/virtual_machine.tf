resource "aws_instance" "gambley_swarm_master" {
  ...
  user_data                   = file("${path.module}/data/user_data/swarm_master_node_setup.sh")
...
resource "aws_instance" "gambley_bastion_node" {
  ami                         = "ami-0c1a7f89451184c8b"
  instance_type               = "t3a.small"
  key_name                    = aws_key_pair.gambley_ssh_key.key_name
  associate_public_ip_address = true
  user_data                   = file("${path.module}/data/user_data/install_nginx.sh")
  security_groups             = [aws_security_group.gambley_bastion_security_group.name]
  tags = {
    "Name"      = "Gambley Bastion Node"
    "Terraform" = "True"
  }
}
...
resource "aws_security_group" "gambley_bastion_security_group" {
  name        = "Gambley Security group"
  description = "Security group for gambley bastion host"
  egress {
    from_port = 0
    to_port   = 0
    protocol  = "-1"
